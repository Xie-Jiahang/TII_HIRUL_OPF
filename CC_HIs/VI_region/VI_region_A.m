...
% only run 100h
clear;clc;rng(7);
folder='C:\Users\jiahang001\OneDrive - Nanyang Technological University\extension\CC_binary_search';
...
load(strcat('../data/con_extreme_point_',num2str(sce),'.mat'));
sample_size=64;
...
save(strcat('../data/CC_VI_regiona',num2str(sce),'_',num2str(Neq0),'.mat'), 'data', '-v7.3');

for j=1:sample_size
    I_d=unifrnd(2.3*0.5,extreme_point(Neq0/50+1,3));
    data.Id.I_d=[data.Id.I_d;I_d];
...
for j=1:sample_size
    I_c=unifrnd(2.3*0.5,extreme_point(Neq0/50+1,2));
    data.Ic.I_c=[data.Ic.I_c;I_c];
